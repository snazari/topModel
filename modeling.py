...
from config import config_processing, config_models, config_saving
...
class Model(object):
    ...
    def load_model(self):
        model_path = make_dirs('model', self.model_name)
        self.model = self.model.load(os.path.join(model_path, 'model.pickle'))
...
if __name__ == '__main__':
    ...
    with open('message.csv', encoding='utf-8') as f:
        ...
        for row in reader_file:
            model(row[1], row[0])
    model.train_model()
    model.get_top_words_from_topic(5)
